...
def t_three_tiers_are_clean():
    ...
    """
    must = [r for r, _w in cs.MUST] + [r for r, _w in cs.MUST_CODE]
...
def t_detects_one_byte_change():
    ...
    try:
        ...
        p = root / "config" / "link.yaml"
        p.write_text(p.read_text(encoding="utf-8") + "x", encoding="utf-8")
        rows = cs.check(root)
        check(rows[0]["level"] == "bad", "改了 link.yaml 却没报不一致：%s" % rows)
        check("link.yaml" in rows[0]["detail"],
              "没说是哪个文件：%s" % rows[0]["detail"])

        # 偏好类（ui.yaml）不一致只该是提示，不是错
        p.write_text("内容 config/link.yaml\n", encoding="utf-8")   # 还原
        p2 = root / "config" / "ui.yaml"
        p2.write_text(p2.read_text(encoding="utf-8") + "x", encoding="utf-8")
        rows = cs.check(root)
        check(rows[0]["level"] == "warn" and "ui.yaml" in rows[0]["detail"],
              "偏好类不一致应当是提示：%s" % rows)
    finally:
...
def t_preflight_exit_codes():
    ...
    try:
        ...
        # 偏好类（ui.yaml）不一致**不该**算漂移：不该弹窗拦人
        p.write_text("内容 config/link.yaml\n", encoding="utf-8")
        q = root / "config" / "ui.yaml"
        q.write_text(q.read_text(encoding="utf-8") + "x", encoding="utf-8")
        check(cs.preflight(root) == 0, "偏好类不一致不该返回 3")
    finally:
        shutil.rmtree(str(root), ignore_errors=True)

...
#: 工作台在跑时，GPU 利用率高于它 ⇒ 判"卡上真有活在算"（多半就是实时预览的推理）。
#: **不能沿用上面那个 70%**：实测 2026-09-26，工作台预览开着时利用率只有 **36%**，
#: 探针量到 64×64 = 18.4 ms、全图 = 19.8 ms（干净基线 10.4 / 10.5）—— 第 3 节
#: 正确地说了"这是环境，不是模型"，可第 0 节却写"本机是干净的，可以跑完整探针" ✗，
#: 同一份输出自相矛盾（当着用户的面发生）。36% 对一张 5070 已经意味着有东西在算。
CLEAN_GPU_PCT = 15.0
...
def report_load():
    ...
    if g is None:
        ...
        if u >= 70:
            ...
            clean = False
        elif d["app_mb"] is not None and u >= CLEAN_GPU_PCT:
            # **工作台自己就是最大的那个占用者**（实时预览在跑推理）。
            # 这条是 2026-09-26 现场加的：当时 36% 没被认出来，于是第 0 节说"干净"、
            # 第 3 节说"这是环境"，同一份输出自己打自己。
            print("   ← 工作台在跑、卡上也有活（利用率 %.0f%%）：**这多半就是实时预览**"
                  % u)
            print("      ⇒ 这不是干净基线（下面 64×64 与全图会一起偏慢）。")
            print("      要干净基线：先停掉实时预览（或关掉工作台）再跑。")
            clean = False
        else:

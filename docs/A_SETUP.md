...
上面这些命令（**对时 / 探针 / 键盘中继 / 推流**，以及寻路要用的**小地图推流**）
都做成了按钮，装在 A 机上直接跑 —— **A 机上不需要敲任何命令**。
...
**依赖装进哪个解释器**：装进「启动部署台的那个」就够了 —— 五个服务都由部署台用
...
| 放行入站 TCP 5003 | 小地图推流（寻路定位用） | 同 §1，把 `protocol=UDP` 改成 `TCP`；只在用寻路时才需要 |
...
- **五张服务卡片**（顺序就是部署顺序）：参数、启动/停止、状态、已运行时长
- **环境自检**：ffmpeg 在不在 / 有没有 NVENC / 串口列不列得出来 / B 机通不通 /
  证书在不在 / 小地图区域框了没有 / **参数和 `config/link.yaml` 对不对得上**
  （最后一条最省钱：不一致时的现象是「流发出去了一切正常，B 机就是没画面」）
- **运行日志**：几个服务的输出混在一起，可以按服务筛、可以存文件
- **每张卡片底部写着「将执行」的完整命令行**，可复制 —— 界面上做的任何事都等价于
  手敲那条命令，想手工排查时直接贴到 PowerShell 里跑

### 小地图推流（只在用寻路定位时才起）

B 机工作台的「路线识别 → 小地图定位」要靠小地图上的玩家黄点算世界坐标，
而主画面那一路已经压过一遍，黄点只有 2~5 像素、压完就糊了 —— 所以这一张卡片
把**小地图面板单独截屏**推给 B 机。用法：

1. 进游戏，把小地图露出来（别被对话框挡）；
2. 部署台「小地图推流」卡片 →「小地图区域」右边点 **「框选…」**
   （点下去部署台会**自己藏起来**再抓屏，所以框到的是游戏画面）；
   屏幕上把**小地图面板本身**框出来 —— 只框面板：多框进来的血条/聊天会一起推过去，
   既费带宽，又会让 B 机「底图对不上」，看着像寻路坏了；
3. 点「启动」。日志里出现 `监听 0.0.0.0:5003 … 等 B 机连进来…` 就对了；
   之后 B 机跑一次 `python -m perception.minimap --map <地图id>` 去量换算。

注意：区域是**屏幕坐标**，所以游戏窗口挪过位置/换过分辨率或显示缩放/改过 UI 布局
之后要重新框一次；这一路基本不吃 CPU（10 fps 的小图 JPEG），不用寻路时不用起它。
